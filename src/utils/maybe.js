 ...
 * @param selector {string} - (optional) used to safely pick a deep value from obj.
...
function Maybe (obj, selector, def) {
  ...
  if (!(this instanceof Maybe)) {
    return new Maybe(obj, selector, def)
  }
...
Maybe.prototype.bind = function (fn) {
  ...
  const ret = fn.call(this, this._value)
...
Maybe.prototype.invoke = function (fnName) {
  if (this.isNothing()) { return this }
  const fn = this.get(fnName).value(null)
  const args = (arguments.length >= 2) ? [].slice.call(arguments, 1) : []
  const ret = (fn == null) ? null : fn.apply(this.value_, args)
  if (ret === undefined) { return this }
  return new Maybe(ret, null, this._default)
}
...
Maybe.prototype.isEqual = function (other) {
  const comparitor = new Maybe(other)
...
Maybe.prototype.toJSONString = function (def, replacer, space) {
  const val = this.value(def || this._default || {})
